...
const targetStr = `      {/* Main Categories */}
          ...
          return (
            ...
            >
              ...
              <div className="relative z-10 max-w-[60%]">
                <h2 className="text-3xl font-pixel text-black mb-3">{cat.name}</h2>
              ...
              <div className="relative z-10 w-24 h-24 flex items-center justify-center">
                {cat.icon ? (
                  ...
                  <Zap className="w-16 h-16 text-primary opacity-80 group-hover:scale-110 group-hover:rotate-6 transition-all duration-300" />
                )}
              </div>
            </Link>
          );
...
const replaceStr = `      {/* Main Categories */}
...
const t2 = `{(settings.categories?.length > 0 ? settings.categories : [`;
const r2 = `{(settings.categories && settings.categories.length > 0 ? settings.categories : [`;
file = file.replace(t2, r2); // Just in case it was there instead
